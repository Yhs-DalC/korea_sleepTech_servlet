...
* - 웹 요청과 응답의 흐름을 메소드 호출만으로 체계적인 설계를 다룸
*
* 2. 서블릿 컨테이너
...
@WebServlet("/") //서블릿 URL 매핑을 위한 어노테이션
...
public class UserServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        // 현재 요청한 경로 (URL패턴)를 가져옴
        String action = request.getServletPath();

        try{
            // URL 경로에 따라 서로 다른 메소드 호출
            switch (action) {
    ...
    // POST 요청 처리 메소드(POST 요청을 GET 요청처럼 처리)
    @Override
